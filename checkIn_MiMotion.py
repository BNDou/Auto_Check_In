...
class MiMotion:
    ...
    def get_time(self):
        try:
            ...
            print(f"时间戳获取失败，使用本地时间: {str(e)}")
            return str(int(time.time() * 1000))

    def login(self, phone, password):
        """
        登录方法，增加完整的异常捕获
        返回：(login_token, userid, app_token, error_msg)
        """
        ...
        try:
            r1 = requests.post(url1, data=data1, headers=self.headers, timeout=15)
            ...
            try:
                ...
                return 0, 0, "", f"登录接口返回非JSON数据: {str(e)}，响应内容: {r1.text[:200]}"
            
            # 检查是否有access字段
            if "access" not in r1_json:
                return 0, 0, "", f"登录接口返回无access字段，响应: {str(r1_json)}"
            
            code = r1_json["access"]
        except requests.exceptions.RequestException as e:
            return 0, 0, "", f"登录请求异常: {str(e)}"
        except Exception as e:
            return 0, 0, "", f"登录失败: {str(e)}"
        ...
        try:
            r2 = requests.post(url=url2, data=data2, headers=self.headers, timeout=15)
            r2_json = r2.json()
            # 检查token_info字段
            if "token_info" not in r2_json:
                return 0, 0, "", f"获取token失败，无token_info字段: {str(r2_json)}"
            
            login_token = r2_json["token_info"]["login_token"]
            userid = r2_json["token_info"]["user_id"]
            app_token = r2_json["token_info"]["app_token"]
            return login_token, userid, app_token, ""
        except KeyError as e:
            return 0, 0, "", f"获取token失败，缺少字段: {str(e)}，响应: {str(r2.json()) if r2.status_code == 200 else r2.text[:200]}"
        except json.JSONDecodeError as e:
            return 0, 0, "", f"Token接口返回非JSON数据: {str(e)}，响应内容: {r2.text[:200]}"
        except requests.exceptions.RequestException as e:
            return 0, 0, "", f"Token请求异常: {str(e)}"
        except Exception as e:
            return 0, 0, "", f"获取token失败: {str(e)}"

    def main(self):
        """
        主方法，增加完整的异常捕获，确保每个错误都能被记录
        """
        # 先检查必要的字段
        required_fields = ["phone", "password"]
        for field in required_fields:
            if field not in self.check_item or not self.check_item.get(field):
                error_msg = f"缺少必要字段: {field}"
                msg = [
                    {"name": "帐号信息", "value": "未知账号"},
                    {"name": "修改信息", "value": error_msg},
                ]
        ...
        try:
            ...
            print(f"初始化步数失败: 已将最小值设置为 10000，错误: {str(e)}")
            min_step = 10000
        try:
            max_step = int(self.check_item.get("max_step", 19999))
        except Exception as e:
            print(f"初始化步数失败: 已将最大值设置为 19999，错误: {str(e)}")
            max_step = 19999

        step = str(random.randint(min_step, max_step))
        
        # 调用登录方法，获取详细错误信息
        login_token, userid, app_token, login_error = self.login(phone, password)

        if login_error or login_token == 0:
            msg = [
                {"name": "帐号信息", "value": f"{phone[:4]}****{phone[-4:]}"},
                {"name": "修改信息", "value": f"登录失败: {login_error}"},
            ]
        else:
            try:
                t = self.get_time()
                ...
                # 处理正则匹配失败的情况
                try:
                    date_match = finddate.findall(data_json)[0]
                    step_match = findstep.findall(data_json)[0]
                    data_json = re.sub(date_match, today, str(data_json))
                    data_json = re.sub(step_match, step, str(data_json))
                except IndexError as e:
                    raise Exception(f"正则匹配日期/步数失败: {str(e)}")
                ...
                msg = [
                    {"name": "帐号信息", "value": f"{phone[:4]}****{phone[-4:]}"},
                    {"name": "修改信息", "value": f"修改步数失败: {str(e)}"},
                ]
...
if __name__ == "__main__":
    ...
    log = ""
    cookie_mimotion = get_env()
    print(f"✅ 检测到共 {len(cookie_mimotion)} 个小米运动账号\n")

    # 遍历账号时增加独立的异常捕获，确保一个账号失败不影响其他账号
    for idx, data in enumerate(cookie_mimotion, 1):
        try:
            # 清理空数据
            data = data.strip()
            if not data:
                print(f"❌ 第{idx}个账号数据为空，跳过")
                log += f"第{idx}个账号：数据为空，跳过\n\n"
                continue
            ...
            # 检查必要字段
            if not user_data.get("phone") or not user_data.get("password"):
                error_msg = f"缺少phone或password字段，数据：{data}"
                print(f"❌ 第{idx}个账号{error_msg}")
                log += f"第{idx}个账号：{error_msg}\n\n"
                continue
            
            # 执行修改步数
            account_log = MiMotion(user_data).main()
            log += f"第{idx}个账号：\n{account_log}\n\n"
            print(f"✅ 第{idx}/{len(cookie_mimotion)}个账号处理完成：{user_data['phone']}")
            
            # 延迟，避免请求过快
            if idx != len(cookie_mimotion):
                print("⏳ 正在等待130秒，继续下一个账号...")
                time.sleep(130)
                
        except Exception as e:
            error_msg = f"第{idx}个账号处理异常: {str(e)}"
            print(f"❌ {error_msg}")
            log += f"{error_msg}\n\n"
            # 异常后也延迟，避免请求过快
            if idx != len(cookie_mimotion):
